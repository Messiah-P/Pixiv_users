...
class DataOutPut(object):
    ...
    def download_file(self, id, file_url, file_path, userName, illustTitle):
        # 将文件二进制保存为本地文件
        # 获取二进制数据
        ...
        is_contain_ch="/" in illustTitle
        if is_contain_ch:
            illustTitleList = illustTitle.split('/')
            illustTitle = illustTitleList[0] +  '-' + illustTitleList[1]
    ...
    def open_zip(self, file_path, file_name, illust_id, userName, illustTitle):
        # 解压压缩包
        temp_file_list = []
        file_path = os.path.join(file_path, userName)
    ...
    def create_gif(self, file_list, delays, file_path, illust_id, userName, illustTitle, file_name):
        # 传入文件列表与延迟时间列表，合成gif
        file_path = os.path.join(file_path, userName)
    ...
    def ugoira_download(self, resource, file_path):
        ...
        delays = t[1]
        # 将压缩包下载到本地，获取压缩包名称
        file_name = self.download_file(illust_id, zip_file, file_path, resource[5], resource[6])
        # 解压压缩包，获取解压后的文件列表
        file_list = self.open_zip(file_path, file_name, illust_id, resource[5], resource[6])
        # 合成gif
        self.create_gif(file_list, delays, file_path, illust_id, resource[5], resource[6], file_name)
        # 删除中间文件和文件夹
        for img in file_list:
            os.remove(img)
    ...
    def static_download(self, resource, file_path):
        ...
        for url in original_list:
            self.download_file(resource[0], url, file_path, resource[5], resource[6])
